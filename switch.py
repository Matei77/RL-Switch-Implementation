...
class BPDU:
    def __init__(self, src_mac, root_bridge_id, sender_bridge_id, root_path_cost, interface, message_age = 0, max_age = 20, hello_time = 0, forward_delay = 0,):
    ...
    def create_package(self):
        ...
        bpdu_package = struct.pack("!6s6sHBBBIBQIQH", self.dst_mac, self.src_mac, self.llc_length, 
                                   ...
                                   self.root_bridge_id, self.root_path_cost, self.sender_bridge_id, self.port_id)
...
def check_bpdu_package(data, own_bridge_ip, root_info, trunk_interfaces_states):
    # get root_bridge_id and root_path_cost from mutable container in order to reflect changes
    root_bridge_id = root_info[0]
    root_path_cost = root_info[1]
...
def forward_package_from_access(recv_interface, data, new_data, length, mac_table, interfaces, vlan_map):
...
def forward_package_from_trunk(recv_interface, data, new_data, length, mac_table, interfaces, vlan_map):
    dest_mac, src_mac, ethertype, vlan_id = parse_ethernet_header(data)
...
def forward_package(recv_interface, data, length, mac_table, interfaces, vlan_map):
    ...
    if vlan_id == -1:
        forward_package_from_access(recv_interface, data, new_data, length, mac_table, interfaces, vlan_map)
    
    # came from trunk interface
    else:
        forward_package_from_trunk(recv_interface, data, new_data, length, mac_table, interfaces, vlan_map)
...
def main():
    ...
    while True:
        ...
        if data[0:6] == bytes.fromhex("0180C2000000"):
            check_bpdu_package(data, own_bridge_id, root_info, trunk_interfaces_states)
